...
種目別の記録詳細は新しい順にまず5件を表示し，「もっと見る」で次の5件を下に追加します．
表示済みの記録とスクロール位置は維持します．
日付の下にコンディション，セット行のRMの右に補助の有無を表示します．
...
記録入力中の下書きは，同じブラウザ内へ自動保存します．
再読み込みや画面移動後も，同じアカウントの日付別の入力画面で復元できます．
チェックボタンで確定するまでは，トレーニング記録や集計には反映しません．

下書きは端末間で同期されず，ブラウザのデータを消去すると失われます．
種目と重量・回数などを入力して左上の×を押すと，保存せずに閉じるか確認します．
保存せずに閉じると，その入力内容は端末内からも削除します．
チェックで保存した後は入力画面に留まり，次の種目を追加できます．
確定した記録の正本はSupabase PostgreSQLです．

## 目標の振り返り

期限当日以降，ホームと設定の目的・目標に振り返りへの案内を表示します．
アプリ内での案内であり，バックグラウンド通知ではありません．

対象期間のトレーニング日数，セット数，種目ごとの最高重量を参考に，達成状況，自己評価，振り返り，次の行動を記入できます．
集計開始日は変更可能です．
振り返りと次の目標をまとめて保存し，過去の振り返りは設定の目的・目標から確認できます．
